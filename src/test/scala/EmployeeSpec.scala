...
class EmployeeSpec extends AsyncFunSuite with EmployeeRepo with H2DBComponent {
  ...
  test("should update a record in the table (Updation in a tuple)") {
    val res = updateTuple(2,("Ishant",3.5))
    res.map{ x => assert(x == 1)}
  }
  ...
  test("should generate the list of employees") {
    val list = getAll
    assert(Await.result(list, Duration.Inf) ==  List(Employee(1,"Gitika",2.5), Employee(2,"Raman",2.0),
      Employee(3,"Anuj",3.0), Employee(4,"Jatin",3.5), Employee(5,"Anmol",4.0)))
  }

  test("should give the max employee experience") {
    val res = getAvgEmpExp
    res.map { x => assert(x == Some(4.0)) }
  }

  test("should insert an employee using plain sql") {
    val res = insertEmployee
    res.map{ x => assert(x==1) }
  }
